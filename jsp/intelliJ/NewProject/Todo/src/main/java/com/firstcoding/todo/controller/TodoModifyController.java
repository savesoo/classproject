...
@WebServlet(name = "TodoModifyControllers", value = "/todo/modify")
public class TodoModifyController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 사용자가 입력했던 데이터를 기본값으로 가지는 입력 form 화면
        String tno = request.getParameter("tno");
        System.out.println("사용자 요청 번호 : " + tno);

        // 수정할 할일의 번호(tno) 받아서 Service에게 전달 -> TodoDTO 받음
        TodoDTO todo = new TodoDTO(1, "숙제", "2022-10-30", false);
        request.setAttribute("todo", todo);

        RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/todotodo/modify.jsp");
