...
-- as : 서브쿼리를 이용해서 기존 테이블 구조로 생성, 데이터 저장, 단 제약 조건은 카피되지 않는다!
create table emp02
as
select * from emp
;
...
-- 테이블의 모든 행을 삭제 : 물리적 삭제 -> 복구 불가
truncate table emp03;

desc emp03;

-- 테이블의 이름 변경 : rename 기존테이블 이름 to 새로운 이름
select * from emp03;
...
create table emp01 (
        empno number(4) not null unique, -- not null 제약조건은 컬럼 정의 위치에서 정의
...
insert into emp01 (empno, ename, job, deptno) values (1112, 'LEE', 'MANAGER', 20);
insert into emp01 (empno, ename, job, deptno) values (null, 'LEE', 'MANAGER', 20); -- 제약조건에 위배
select * from emp01;
...
desc emp01;
insert into emp01 (empno, ename, job, deptno) values (1111, 'KIM', 'DEV', 10);
insert into emp01 (empno, ename, job, deptno) values (1112, 'KIM', 'DEVs', 60);
insert into emp01 (empno, ename, job, deptno) values (null, 'LEE', 'MANAGER', 20);
select * from emp01;
...
insert into emp01 values(1111, 'KIM', 'MANAGER', 100, 'A', 20); -- 조건에 위배됨
insert into emp01 values(1111, 'KIM', 'MANAGER', 3400, 'M', 20);

desc emp01;


-- default : null 값 입력시 기본으로 저장할 값을 정의
create table emp01 (
        empno number(4) constraint emp01_empno_pk primary key,
        ename varchar2(10) constraint emp01_ename_NN not null,
        job varchar2(9)constraint emp01_job_UK unique,
...
insert into emp01 (empno, ename, sal, gender, job, deptno) values (1111, 'KIM', 3400, 'M', 'MANAGER', 20);
insert into emp01 (empno, ename, sal, gender, job, deptno, hiredate) values (1111, 'KIM', 3400, 'M', 'MANAGER', 20, sysdate);
select * from emp01;
...
create table emp01 (
        ...
        gender char(1) constraint emp01_gender_CK check(gender in('M', 'F')) , 
        deptno number(2)constraint emp01_deptno_FK references dept(deptno),
        hiredate date -- 없으면 null 들어감. 따로 넣어줄 수 있으나 디폴트값 있는 게 좋음.
