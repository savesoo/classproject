...
from emp
where comm is not null and comm != 0    -- 0은 커미션이 없는 것이므로 부정조건 추가
order by sal desc, comm desc ;          -- 둘 다 내림차순 정렬이므로 각각 desc 붙여주기
...
-- where (job='CLERK' or job='SALESMAN') and sal not in(1600, 950, 1300) 형태도 가능~

-- 15. 커미션이 $500 이상인 사원의 이름과 급여 및 커미션을 출력하시오.
select ename, sal, comm
from emp
where comm >= 500 ;
-- null은 false 값이므로 비교 없이 넘어감
